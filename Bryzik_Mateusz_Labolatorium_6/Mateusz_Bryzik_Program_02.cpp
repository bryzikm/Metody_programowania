...
int main()
{
	int n, i, j, max=0, x, y, z;	//deklaracja zmiennych n - l. krawedzi, i, j - zmienne indeksowe, max - zmienna przechowywujaca max wierzcho³ek
	...
	for(i = 0; i < n; i++)
  {
    plik_we >> x >> y >> z; // odczytujemy krawêdŸ
	...
	nowy->weight = z; //przypisujemy wartoœæ wadze z pobranej zmiennej
  ...
  for(int i = 0; i < max; i++)
  {
    ...
    {
      cout << nowy->value <<"("<<nowy->weight<<")"<< " "; //wy³uskanie value oraz weight
...
int fmax(fstream &plik_we)
	{
		int i, j, l, n, max = 0; //zmienne: n - l. zestawow krawedzi, i, j - indeksy, max - maxymalny wierzcholek

				plik_we>>n;
		
		for(int k=0; k<n; k++) //pêtla ktora obraca siê n razy (liczba krawedzi)
		{
			plik_we>>i>>j>>l; //wczytanie pary liczb polaczonych krawedzia z pliku
...
void wypis(int **T,int max)
	{
		for(int i=0; i<max; i++) //pêtla odpowiedzialna za przeskok do kolejnego wiersza
		{
			...
			{
				if(T[i][j]>0) //sprawdzenie czy krawedz miedzy indeksami (wierzcholkami) istnieje
					{
						cout<<T[i][j]<<" "; //wypis
					}
				else
...
void przypisz(fstream &plik_we, int **T)
	{
		...
		plik_we>>n;//wczytanie liczby zestawow krawedzi grafu
				...
				{
					plik_we>>i>>j>>l; //wczytanie 2 wierzcholkow z pliku

					T[i-1][j-1] = l; //przypisanie w macierzy wartosci 1 dla wartosci indeksow polaczonych krawedziami
